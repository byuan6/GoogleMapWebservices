...
namespace BlockDivision
{
    public partial class Form1 : Form
    {
        ...
        {
            this.Text = "Representation of distance request. Each pixel on y-axis is a origin, and each pixel on x-axis is destination.";
        ...
        private void run()
        {
            ...
            for (int j = 1; j < 30; j++)
                ...
                ReduceSurfaceAreaByReordering waterdrop = new ReduceSurfaceAreaByReordering() { Area = area };
                this.Invoke(new MethodInvoker(delegate()
                {
        ...
        Bitmap Clone(Bitmap bmp)
        {
            using (var ms = new System.IO.MemoryStream())
            {
                bmp.Save(ms, System.Drawing.Imaging.ImageFormat.Bmp);
                ms.Position = 0;
                Bitmap clone = new Bitmap(ms);
                return clone;
            }
